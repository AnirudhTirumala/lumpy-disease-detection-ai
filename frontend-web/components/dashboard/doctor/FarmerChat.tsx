...
import { Send, Image, Mic, MicOff, Video, Play, Pause, X, Search } from 'lucide-react';
...
interface Thread {
  threadId: string; otherId: string; otherName: string;
...
export default function FarmerChat() {
  ...
  // KEY FIX: use a ref to track the "has user manually selected" state,
  // so the polling loadThreads() never auto-overwrites their choice.
  ...
  const loadThreads = useCallback(() => {
    ...
    fetch(`/api/chat?userId=${user.id}`)
      ...
      .then(d => {
        ...
        setThreads(list);
        // Only auto-select the first thread ONCE, and only if user hasn't picked one yet
  ...
  function selectThread(threadId: string) {
    userHasSelectedRef.current = true; // lock in the manual choice
    setActiveThreadId(threadId);
    setMessages([]); // clear immediately to avoid showing old thread's messages briefly
  }
  ...
  async function handleText(e: React.FormEvent) {
    ...
    fd.append('senderName', user.name); fd.append('senderRole', 'doctor');
    ...
    <div className="max-w-5xl mx-auto h-[calc(100vh-12rem)] flex rounded-2xl overflow-hidden border border-hairline shadow-card">
      ...
      <div className="w-72 shrink-0 flex flex-col border-r border-gray-200 bg-white">
        <div className="px-4 py-3 bg-[#075E54]">
          <p className="text-white font-bold text-base">Farmer Chats</p>
          <p className="text-white/70 text-xs">{threads.length} conversation{threads.length !== 1 ? 's' : ''}</p>
        </div>
            ...
            <button
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-gray-800 truncate">{t.otherName}</p>
                  {t.lastMessageTime && (
          ...
          <div className="flex-1 flex items-center justify-center" style={{ background: '#F0F2F5' }}>
            <div className="text-center">
              <div className="w-24 h-24 rounded-full bg-[#075E54]/10 flex items-center justify-center mx-auto mb-4">
                <Send className="w-10 h-10 text-[#075E54]/30" />
              </div>
                  ...
                  {group.messages.map(m => {
                    ...
                    return (
                      <div key={m.id} className={`flex mb-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[72%] rounded-2xl px-3 py-2 shadow-sm ${
                          isMine ? 'bg-[#DCF8C6] rounded-tr-sm' : 'bg-white rounded-tl-sm'
                        }`}>
                          {!isMine && <p className="text-[10px] font-bold text-[#075E54] mb-0.5">{m.senderName}</p>}
                          {m.type === 'text' && <p className="text-sm text-gray-800 leading-relaxed break-words">{m.text}</p>}
                          {m.type === 'image' && m.fileUrl && (
                            <img src={m.fileUrl} alt="img" className="max-w-[200px] rounded-xl cursor-pointer hover:opacity-90 mb-1"
                              onClick={() => window.open(m.fileUrl!, '_blank')} />
                          )}
                          {m.type === 'voice' && m.fileUrl && (
                            <div className="flex items-center gap-2 min-w-[180px]">
                              <button onClick={() => toggleAudio(m.id, m.fileUrl!)}
                                className="w-8 h-8 rounded-full bg-[#075E54] flex items-center justify-center text-white">
                                {playingId === m.id ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
                              </button>
                              <div className="flex-1 h-1 bg-gray-300 rounded-full"><div className="h-1 bg-[#075E54] rounded-full w-1/3" /></div>
